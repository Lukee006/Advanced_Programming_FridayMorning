...
class Order:
    ...
    def receipt(self):
        lines = []
        lines.append("Receipt for " + self.customer.name + " (" + self.customer.tier_name() + ")")
        lines.append("-" * 40)

        for item in self.items:
            line = item.product.price * item.quantity
            lines.append(item.product.name + " x" + str(item.quantity) + " = " + str(line))
